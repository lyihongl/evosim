...
public:
  ...
  void main_loop();
  void draw_angled_line(sf::RenderTarget &target, const sf::Vector2f &position,
                        const double length, const int angle);
  std::unordered_map<std::string, std::string> debug_info;
  render_engine(int width, int height, std::string title, omni_sight &os);
  template <typename T>
  void draw_to_target(std::vector<sf::RenderTexture> &targets,
                      std::size_t target, T t);
  // sf::Image &get_window_image();
  // uint32_t get_pixel(unsigned int x, unsigned int y);
};

template <typename T>
void render_engine::draw_to_target(std::vector<sf::RenderTexture> &targets,
                                   std::size_t target, T t) {
...
render_engine::render_engine(int width, int height, std::string title,
                             omni_sight &os)
    : os{os}, window{sf::VideoMode(width, height), title,
...
void render_engine::main_loop() {
  ...
  std::chrono::time_point<std::chrono::system_clock> time_end =
      std::chrono::system_clock::now();
  std::chrono::time_point<std::chrono::system_clock> time_end_fps =
      std::chrono::system_clock::now();
  ...
  for (; window.isOpen();) {
    ...
    ImGui::Text(std::string("Longest Alive Index: " +
                            std::to_string(os.longest_alive_index))
                    .c_str());
    ...
    for (int i = 0; i < os.am->num_agents; i += 2) {
      ...
      // "<<os.am->num_agents <<" it: "<<*it);
      if (os.am->active_agent_set.find(i) == os.am->active_agent_set.end())
        continue;
      ImGui::PushStyleColor(
          ImGuiCol_Text, ImVec4(ImColor(os.am->colors[i].r, os.am->colors[i].g,
                                        os.am->colors[i].b)));
      ImGui::Text(std::string("[=] ").c_str());
      ImGui::SameLine();
      ImGui::PopStyleColor();
      ImGui::Text(
          std::string("Agent: " + std::to_string(i) +
    ...
    //    os.render_targets[i].getTexture().getNativeHandle());
    //    glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE,
    //    os.screen_dat[i].get()); glBindTexture(GL_TEXTURE_2D, 0);
    //}

    // auto functor = [](const omni_sight &os, const int &i) -> void {
    //    glBindTexture(GL_TEXTURE_2D,
    //    os.render_targets[i].getTexture().getNativeHandle());
    //    glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE,
    //    os.screen_dat[i].get()); glBindTexture(GL_TEXTURE_2D, 0);
    ...
    // glBindTexture(GL_TEXTURE_2D,
    // os.render_targets[0].getTexture().getNativeHandle());
    // glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE,
    // os.screen_dat.get());
    ...
    //        log(j<<" "<<i<<" "<<(int)os.screen_dat[i*1600+j].r<<"
    //        "<<(int)os.screen_dat[i*1600+j].g<<"
    //        "<<(int)os.screen_dat[i*1600+j].b);
    //    }
    //}
    // break;

    // window_contents_texture.display();
    // window_contents_image =
    // window_contents_texture.getTexture().copyToImage();
    // log((int)window_contents_texture.getTexture().copyToImage().getPixel(100,
    // 100).r);
...
void render_engine::draw_angled_line(sf::RenderTarget &target,
                                     const sf::Vector2f &position,
                                     const double length, const int angle) {
  // angle = evo_math::normalize_angle(angle);
  // sf::Vector2f end{position.x +
  // evo_math::cos(evo_math::normalize_angle(angle)) * length, position.y -
  // evo_math::sin(evo_math::normalize_angle(angle)) * length};
  sf::Vector2f end{position.x + evo_math::cos(angle) * length,
                   position.y - evo_math::sin(angle) * length};
  sf::Vertex line[] = {position, end};
  target.draw(line, 2, sf::Lines);
}

// sf::Image &render_engine::get_window_image() {
//    window_contents_image =
//    window_contents_texture.getTexture().copyToImage(); return
//    window_contents_image;
//}
