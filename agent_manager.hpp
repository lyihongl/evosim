...
agent_manager::agent_manager() : num_agents{0} {}

void agent_manager::add_agent(sf::Vector2f &p, sf::Color &t, MLP &m,
  ...
  if (empty_slots_stack.size() == 0) {
    ...
    // auto it = std::find_if(sorted_agent_set.begin(), sorted_agent_set.end(),
    // [&](const std::pair<unsigned int, std::size_t>& val)-> bool {
...
void agent_manager::remove_agent(std::size_t i) {
  if (active_agent_set.find(i) == active_agent_set.end())
    return;
