...
int main() {
  ...
  // std::vector<std::vector<sf::Vector2f>> scan_points(8,
  // std::vector<sf::Vector2f>(30)); evo_math::start_scan_positions(start_scan,
  // scan_angle, 12); for (int j = 0; j < start_scan.size(); j++) {
  //    start_scan[j] += {100, 800};
  //    evo_math::populate_line_points(scan_points[j], 30, start_scan[j], 300,
  //    scan_angle[j]); log("start scan: " << scan_angle[j] << " " <<
  //    start_scan[j].x << " " << start_scan[j].y);
  //}
  // for (int k = 0; k < scan_points.size(); k++) {
  //    for (int j = 0; j < scan_points[k].size(); j++) {
  //        log_precision("points: " << k << " " << j << " " <<
  //        scan_points[k][j].x << " " << scan_points[k][j].y, 6);
  //    }
  //}

  //

  std::unique_ptr<agent_manager> am =
      std::unique_ptr<agent_manager>(new agent_manager{});
  ...
  // for (auto it = am->active_agent_set.begin(); it !=
  // am->active_agent_set.end(); it++) {
  //    log("it: " << *it);
  //}
